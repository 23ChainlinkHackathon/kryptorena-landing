...
const About = (props) => {
  ...
  return (
    <div id="about" className="lg:w-auto w-screen overflow-hidden bg-[#0F0F0F] lg:h-[700px] md:h-[1700px] xsm:h-[1700px] gap-5 text-white flex xsm:flex-col items-center justify-center">
      ...
      <ul className="flex xsm:flex-col lg:flex-row gap-8">
        {titles.map((title, index) => (
          <li key={index} className="  xsm:w-[325px] xsm:h-[440px]  lg:w-[280px] lg:h-[410px]   flex xsm:flex-col gap-2 justify-center items-center bg-[#242424] ring-1 rounded-[8px] ring-[#F4DDB6] h-[405.33px]">
            <img className=' xsm:w-[280px] xsm:h-[300px] lg:h-[250px] lg:w-[200px] rounded-[4px]' src={imgs[index]} alt={`Placeholder ${index + 1}`}  />
            <h2 className='text-[20px] text-[#F4DDB6] '>{title}</h2>
            <p className= {`text-[12px] ${title === titles[0]? "xl:w-[220px]":""}  xl:h-[48px] leading-[24px] lg:w-[230px] ${title === title2? "xsm:w-[290px] xsm:h-[48px] lg:w-[220px] xl:w-[270px] md:text-[12px]  xsm:text-[14px]" : "xsm:text-[14px] text-center justify-center md:text-[12px] md:w-[357.33px] xsm:w-[280px]"}`}>{texts[index]}</p>
          </li>
        ))}
      </ul>
    </div>
